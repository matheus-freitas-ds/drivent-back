...
describe('GET /booking', () => {
  ...
  describe('PUT /booking/:bookingId', () => {  
    ...
    describe('when token is valid', () => {
      ...
      it('should respond with status 400 if bookingId is not valid', async () => {
        const user = await createUser();
        const token = await generateValidToken(user);
      ...
      it('should respond with status 403 if the new room is full', async () => {
        const user = await createUser();
        const token = await generateValidToken(user);
        const hotel = await createHotel();
        const room = await createRoomWithHotelIdAndCapacityExceeded(hotel.id);
      ...
      it('should respond with status 200 with bookingId data', async () => {
        const user = await createUser();
        const token = await generateValidToken(user);
        const hotel = await createHotel();
        const room = await createRoomWithHotelId(hotel.id);
